# Recovery — Pitch & Demo Script (2-Minute Walkthrough)

### 1. Opening Hook (0:00 - 0:25)
> *"Most payment recovery solutions are just dumb notification spammers or generic chatbots. What you're seeing today is **Recovery** — an autonomous closed-loop multi-agent system that detects revenue at risk, diagnoses root causes, selects compliant interventions, and enforces hard regulatory guardrails with an append-only audit trail."*

### 2. Live Ops & 6-Agent LangGraph (0:25 - 0:50)
> *"Watch the live stream on the **Interventions** tab as we start our 300-event batch. Notice that every case transitions through a strict 6-agent state machine. For a customer whose UPI AutoPay failed due to bank timeout, our Strategy agent formulates an instant smart retry. For an expired card, it generates localized Hinglish WhatsApp copy with a 1-click Razorpay intent link."*
